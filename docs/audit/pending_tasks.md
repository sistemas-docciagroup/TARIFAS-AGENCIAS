...
- [ ] **1.** Remove `pytesseract` and `Pillow` from `requirements.txt` — never imported anywhere.
...
- [ ] **5.** Convert `SAP_CONECTADO = False` (`services/sap_client.py:21`) to env var `SAP_ENABLED`.
